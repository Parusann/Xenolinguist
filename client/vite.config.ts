...
try { sourceRevision = execFileSync('git', ['rev-parse', '--short=12', 'HEAD'], { cwd: path.resolve(__dirname, '..'), encoding: 'utf8' }).trim(); if (execFileSync('git', ['status', '--porcelain', '--', 'client', 'shared'], { cwd: path.resolve(__dirname, '..'), encoding: 'utf8' }).trim()) sourceRevision += '+working' } catch { /* Source archives may omit Git metadata. */ }
