...
describe('POST /api/tts (success contract)', () => {
  it('returns audio/wav on success', async () => {
    ...
    const res = await request(createApp(testSession)).post('/api/tts').send({ text: 'hello' });
  ...
  it('passes the voice through to synthesize', async () => {
    ...
    await request(createApp(testSession)).post('/api/tts').send({ text: 'hi', voice: 'de' });
  ...
  it('accepts a phonemes-only request', async () => {
    ...
    const res = await request(createApp(testSession)).post('/api/tts').send({ phonemes: 'h@l@U' });
    expect(res.status).toBe(200);
