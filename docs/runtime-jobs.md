...
The [research proposal backend](research-proposals.md) permits four read-only tool actions and one structural repair within at most six generation calls. The preview endpoint returns provenance without a write; the workbench review endpoint persists the starting snapshot, completed result or sanitized failure/cancellation. A pending record without an active job appears interrupted after restart and is never resumed automatically. Decisions use revision checks and atomic writes; selected-test compatibility and actual use of the candidate are required before application. Existing free-text chat retention below remains a separate path.
