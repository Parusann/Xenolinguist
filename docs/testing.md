...
The initial W18 Windows source run exceeded the existing five-second deadline in two disk integration tests (audio lifecycle and archive restoration); the induction tests passed. CI now caps server/shared/engine/evaluation test workers at two to reduce concurrent disk pressure. Test deadlines and assertions are unchanged, and no test retries were added. This is a runner resource bound, not a claim about application latency.

The following Windows browser run exposed a separate sequencing issue in the new induction fixture: receiving all export bytes does not mean the server has finished deleting its staging directory. The immediate inspection correctly received `ARCHIVE_BUSY` while cleanup still held the exclusive archive slot. The test now asserts staging cleanup completes before submitting the separate inspection, without retrying failed uploads or changing the server's concurrency contract.
...
The engine adds 18 unit cases for additive and multiplicative/remainder composition, small bases, ordering, linkers, one-comparison abstention, duplicate evidence, contradictory labels, unknown atoms, irregular observations, explicit limits and competing bases. Four experiment tests cover fit/validation/test separation, lexical split separation, counted irregular errors, deterministic source replay, overwrite refusal and tamper rejection. Local source checks pass 336 unit tests and seven tooling tests with three gated native skips.

The new browser flow runs the worker without model calls, retains ambiguous candidates, requests a distinguishing numeral, explicitly saves its observed answer as validation, predicts an unseen form with an arithmetic tree, rejects stale results and preserves validation through reload and archive restoration. The existing W08 failed-save test now compares executable number candidates; its old token-overlap tie becomes a visible base-5 contradiction, while both base alternatives remain inspectable. Independent installed acceptance adds the same number workflow and a seventh project archive. The harness now awaits export staging cleanup before starting inspection, matching the lifecycle boundary exposed during W18. Final measured results and exact-revision CI are recorded in [implementation progress](implementation-progress.md).
