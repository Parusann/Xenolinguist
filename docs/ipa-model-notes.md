...
Before packaging, `stage-runtime.mjs` walks installed production dependencies and available optional dependencies from Transformers, checks their versions against `package-lock.json`, and preserves their nested layout and license files outside ASAR. ONNX's `bin/napi-v6` subtree contains only the build host's supported native target (Windows x64 for release builds), matching the runtime binding loader; foreign platform/architecture binaries are omitted before packaging. Its generated manifest records package versions and lockfile integrity values. Run `npm ci` first: matching a version is not a substitute for npm's package-integrity verification. Electron Builder copies the staged node_modules root explicitly; its ordinary directory copier excludes that root. An after-pack check verifies package identities and the selected ONNX binding; installed acceptance compares every payload file against the build inventory.
